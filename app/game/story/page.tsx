...
export default function StoryPage() {
  ...
  useEffect(() => {
    setExisting(playerId ? getGameResult(playerId, "story") : null);
  }, [playerId]);
  ...
  function submit() {
    ...
    try {
      const outcome = submitGameResult({ playerId, gameKey: "story", answers, score });
      refresh();
      ...
      {currentQuestion && (
        <section className="questionStack">
          <article className="questionCard">
            <span>Story {currentIndex + 1} / {questions.length}</span>
